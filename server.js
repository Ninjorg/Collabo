...
app.post('/signup', async (req, res) => {
    ...
    if (users.find(user => user.schoolmail === schoolmail)) {
        return res.status(400).json({ message: 'User already exists' });
    }
    if (users.find(user => user.username === username)) {
        return res.status(400).json({ message: 'Username taken' });
    }
    await addChatToUser(username, 'general');
    await addChatToUser(username, 'counting');
    await addChatToUser(username, 'homework');
    ...
    socket.on('joinChat', async (chatId) => {
        try {
            ...
            if (chatDoc.exists()) {
                ...
                const tenHoursAgo = new Date(Date.now() - (2.4 * (10 * 60 * 60 * 1000))).toISOString();
        ...
        try {
            ...
            if (userSnapshot.exists()) {
                ...
                if (timeDifference >= 48 * 60 * 60 * 1000) { // 48 hours in milliseconds
                    // Reset streak to 0 if 48 hours have passed
                    await updateDoc(userRef, {
                        streak: [0, timestamp]
                    });
                    ...
                    // Update the last streak timestamp without incrementing streak
                    await updateDoc(userRef, {
                        'streak[1]': timestamp
                    });
                }
